...
const TimelineComponent = () => {
    ...
    const recentOrders = events.filter((event) =>
        (new Date(event.date) >= new Date(currentDate) || (new Date(event.date) < new Date(currentDate) && event.title !== "Delivered")) &&
        (event.status !== "Delivered")
    );

    const previousOrders = events.filter((event) => new Date(event.date) < new Date(currentDate) && event.title === "Delivered");
